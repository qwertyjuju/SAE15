...
class HeaderDict:
    ...
    def get_sorted_ipcount(self, iptype, reverse=0):
        if reverse:
            return sorted(self.ipcount_list[iptype].items(), key=lambda item: item[1], reverse=1)
        else:
            return sorted(self.ipcount_list[iptype].items(), key=lambda item: item[1])
        ...
        for partindex,part in enumerate(linesplit):
            ...
            if part_type == "attr":
                ...
                if part_type == "comparator":
                    ...
                    self["ip dst"], self["dst port"] = create_ip(linesplit[partindex+1][:-1])
...
def write_xl(headers, path):
    ...
    ws1 = wb.active
    ws1.title="headers"
    ws1.append(list(Header.header_content.keys()))
    for header in headers.get_headers():
        ws1.append(list(header.get_values()))
    ws2=wb.create_sheet(title="ip_count")
    sorted_ipcount=headers.get_sorted_ipcount('s',reverse=1)
    for row in sorted_ipcount:
        if row[0]=="_total":
            pass
        else:    
            ws2.append(row)
    chart = BarChart()
    chart.title = "ip count"
    chart.y_axis.title = 'number of packets'
    chart.x_axis.title = 'source ip'
    data = Reference(ws2, min_col=2, min_row=1, max_row=len(sorted_ipcount))
    ips = Reference(ws2, min_col=1, min_row=1, max_row=len(sorted_ipcount))
